    ...
    style: {
      map: (value: string) => {
        ...
        if (
          ...
          tokens.push("modern");
        if (normalized.includes("luxury")) tokens.push("luxury");
        if (normalized.includes("minimal") || normalized.includes("zen"))
          tokens.push("minimalist");
        if (normalized.includes("scandinavian")) tokens.push("scandinavian");
        if (
        ...
        if (
          ...
          tokens.push("warm");
        if (tokens.length === 0) tokens.push("modern");
        return tokens;
      },
    },
  },
  financial: {
    budget: {
      map: (value: number | string) => {
        ...
        if (num <= 10) return num * 10000000; // Cr to minor units
        if (num <= 100) return num * 100000; // Lakhs to minor units
        return num * 100000;
      },
    },
  },
